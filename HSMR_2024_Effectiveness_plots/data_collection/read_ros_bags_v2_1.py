...
class RosbagParser():
    ...
    def single_datapoint_processing(self, file_name): # parses data into the global dictionaries
        ...
        joint_msg = bag.read_messages(topics=['/PSM2/measured_js'])
        ...
        jacobian_spatial = bag.read_messages(topics=['/PSM2/spatial/jacobian'])
        for topic, msg, t in jacobian_spatial:
            jacobian_timestamps.append(t.secs+t.nsecs*10**-9)
            jacobian.append(list(msg.data))

        jaw_msg = bag.read_messages(topics=['/PSM2/jaw/measured_js'])
        ...
        cartesian = bag.read_messages(topics=['/PSM2/measured_cp'])
        for topic, msg, t in cartesian:
            cp_timestamps.append(t.secs+t.nsecs*10**-9)
            cp_x = msg.pose.position.x
            cp_y = msg.pose.position.y
            cp_z = msg.pose.position.z # the sensor is probably most accurate in the z direction
            cp_ow = msg.pose.orientation.w
            cp_ox = msg.pose.orientation.x
            cp_oy = msg.pose.orientation.y
            cp_oz = msg.pose.orientation.z
            cp_pitch = msg.pose.orientation.pitch
            cp_roll = msg.pose.orientation.roll
            cp_yaw = msg.pose.orientation.yaw
            cp.append([cp_x, cp_y, cp_z, cp_ow, cp_ox, cp_oy, cp_oz, cp_pitch, cp_roll, cp_yaw])

        cartesian_velocity = bag.read_messages(topics=['/PSM2/measured_cv'])
        for topic, msg, t in cartesian_velocity:
            cv_timestamps.append(t.secs+t.nsecs*10**-9)
            cv.append([msg.twist.linear, msg.twist.angular])
        ...
        print("Processed cp: count: {}".format(len(cp_timestamps)))
        ...
        if len(cp) > 0:
            try:
                cp_path = Path(self.output) / "cp"
                cp_path.mkdir(mode=0o777, parents=False)
            except OSError:
                print("CP path exists")

        if len(cv) > 0:
            try:
                cv_path = Path(self.output) / "cv"
                cv_path.mkdir(mode=0o777, parents=False)
            except OSError:
                print("CV path exists")        
        ...
        if len(force_sensor) > 0:
            ...
            force_sensor = None
        if len(cp) > 0:
            cp_timestamps = np.array(cp_timestamps) - start_time
            cp = np.column_stack((cp_timestamps,cp))
        else:
            cp = None

        if len(cv) > 0:
            cv_timestamps = np.array(cv_timestamps) - start_time
            cv = np.column_stack((cv_timestamps,cv))
        else:
            cv = None
        ...
        if len(cp) > 0:
            np.savetxt(self.output + "cp/" + file_name + ".csv", cp, delimiter=',')
        if len(cv) > 0:
            np.savetxt(self.output + "cv/" + file_name + ".csv", cv, delimiter=',')
