...
parser.add_argument('-f', '--file_json', type=str, default='rcm_pivot_ccw.json',
...
sub_folder_name = os.path.join(os.getcwd(), os.path.join('data'))
...
#   
    command = "rosbag record -O {0}.bag /{1}/measured_js /{1}/measured_cp /{1}/measured_cv /{1}/jaw/measured_js /{1}/spatial/jacobian /{1}/setpoint_js /measured_cf".format(os.path.join(sub_folder_name, args.config.split('.')[0]),'PSM'+str(PSM_id))
...
for i in range(len(poses)):
    ...
    sys.stdout.flush()
    end_t = time.time()
    delta_t = 0.001 - (end_t - start_t)        ###############################check console rate
